...
def set_attempts(user_id: int, count: int):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("UPDATE users SET attempts_left = %s WHERE user_id = %s", (count, user_id))
    conn.commit()
    cur.close()
    conn.close()
...
def subtract_attempts(user_id: int, count: int):
    ...
    cur = conn.cursor()
    cur.execute("""
        UPDATE users SET attempts_left = GREATEST(0, attempts_left - %s)
        WHERE user_id = %s
        RETURNING attempts_left
    """, (count, user_id))
    result = cur.fetchone()
...
def create_payment(user_id: int, method: str, amount: float, reference: str = None):
    ...
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO payments (user_id, payment_method, amount, reference_id)
        VALUES (%s, %s, %s, %s)
        RETURNING id
...
def save_video_request(user_id: int, input_type: str, dialect: str, 
                       lecture_type: str = None, lecture_subtype: str = None,
                       education_level: str = None, teacher_name: str = None,
                       teacher_outfit: str = None):
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO video_requests 
        (user_id, input_type, dialect, lecture_type, lecture_subtype, 
         education_level, teacher_name, teacher_outfit, status)
        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, 'processing')
        RETURNING id
    """, (user_id, input_type, dialect, lecture_type, lecture_subtype, 
          education_level, teacher_name, teacher_outfit))
