...
def get_connection():
    ...
    تستخدم psycopg2 مع DictCursor لإرجاع النتائج كـ dictionaries.
    """
...
def init_db():
    ...
    تستدعى مرة واحدة عند بدء تشغيل البوت.
    """
    conn = get_connection()
    cur = conn.cursor()
    ...
    # إضافة الأعمدة الجديدة إذا لم تكن موجودة (للتحديثات المستقبلية)
    ...
    # ============================================================
    # جدول طلبات الفيديو (للتتبع والإحصائيات)
    # ============================================================
    cur.execute("""
    ...
    # ============================================================
    # جدول الملاحظات (للميزات المستقبلية)
    # ============================================================
    cur.execute("""
...
def create_user(user_id: int, username: str, full_name: str, referred_by: int | None = None):
    """إنشاء مستخدم جديد أو تحديث بياناته إذا كان موجوداً"""
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
...
def decrement_attempts(user_id: int):
    """خصم محاولة واحدة من رصيد المستخدم"""
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
...
def subtract_attempts(user_id: int, count: int):
    """خصم عدد معين من المحاولات (لا يقل عن 0)"""
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
        UPDATE users
...
def approve_payment(payment_id: int):
    """الموافقة على دفع وإضافة المحاولات تلقائياً"""
    conn = get_connection()
    cur = conn.cursor()
    cur.execute("""
...
def record_referral(referrer_id: int, referred_id: int) -> dict:
    ...
    # تحويل النقاط إلى محاولات إذا وصلت للحد
    attempts_granted = 0
...
# ============================================================
# دوال الملاحظات (للميزات المستقبلية)
# ============================================================
