...
# ═══════════════════════════════════════════════════════════════════════════════
# استخراج النص من PDF - محسنة مع timeout
# ═══════════════════════════════════════════════════════════════════════════════
...
async def analyze_lecture(text: str, dialect: str = "msa") -> dict:
    ...
    prompt = f"""أنت {teacher}. اشرح {dial}. اكتب 15-20 جملة متنوعة لكل قسم. النص: {preview}. الكلمات: {', '.join(keywords[:15])}. أرجع JSON: {{"title": "عنوان", "sections": [{{"title": "", "keywords": ["","","",""], "narration": ""}}]}}"""
    try:
...
async def fetch_image_for_keyword(keyword: str, section_title: str = "", lecture_type: str = "other", image_search_en: str = "") -> bytes:
    ...
    if img:
        return img
    return _make_image(keyword, col)                if r.status == 200: return await r.read()
    except: pass
    return None
