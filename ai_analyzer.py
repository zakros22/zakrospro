...
async def extract_full_text_from_pdf(pdf_bytes):
    ...
    def _extract():
        ...
        return "\n\n".join([p.extract_text() or "" for p in r.pages if p.extract_text()])
    return clean_text(await asyncio.get_event_loop().run_in_executor(None, _extract))
...
async def _ai_gen(prompt, max_t=8192):
    for _ in range(len(_google_keys)*2):
        ...
        for m in ["gemini-2.0-flash", "gemini-2.0-flash-lite"]:
            try:
                r = await asyncio.to_thread(c.models.generate_content, model=m, contents=prompt, config=genai_types.GenerateContentConfig(temperature=0.7, max_output_tokens=max_t))
                return r.text.strip()
