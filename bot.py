...
async def _process_lecture(
    ...
    async with _Q_SEM:
        try:
            ...
            _check_cancelled()
            await upd(52, "🎤 توليد الصوت البشري...")

            async def _voice_heartbeat():
                voice_msgs = [
                    (52, "🎤 الاتصال بخدمة الصوت..."),
            ...
            used_gtts     = voice_res.get("used_fallback", False)

            voice_note = " (gTTS)" if used_gtts else " (ElevenLabs)"
            ...
            try:
                await context.bot.send_message(
                    ...
                    "الحلول:\n"
                    "• أضف مفتاح Google في `GOOGLE_API_KEYS` (مجاني: aistudio.google.com/apikey)\n"
                    "• أضف مفتاح Groq في `GROQ_API_KEY` (مجاني: console.groq.com)\n"
                    "• أضف مفتاح OpenRouter في `OPENROUTER_API_KEY` (مجاني: openrouter.ai/keys)\n"
                    "• أو انتظر غداً لتجديد الحصة",
                    parse_mode="Markdown",
                )
            except Exception:
                pass
...
async def main():
    ...
    if not webhook_url:
        ...
        for _domain in replit_domains.split(","):
            ...
            if _domain.endswith(".replit.app"):
                webhook_url = f"https://{_domain}"
