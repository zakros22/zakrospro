...
user_states = {}; _Q_SEM = asyncio.Semaphore(2); _active_jobs = {}; _active_tasks = {}; _cancel_flags = {}
...
async def receive_content(update, context):
    ...
    if msg.text:
        ...
        if msg.text == "❓ مساعدة": await msg.reply_text("📖 أرسل PDF أو نص طويل"); return
    ...
    if msg.document:
        ...
        try:
            ...
            if ext == "pdf": text = await extract_full_text_from_pdf(bytes(raw))
    ...
    else: await msg.reply_text("⚠️ أرسل PDF أو نص (200 حرف على الأقل)"); return
