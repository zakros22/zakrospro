...
from database import (
    init_db, get_user, create_user, is_banned, decrement_attempts, add_attempts,
    increment_total_videos, save_video_request, update_video_request, record_referral, get_referral_stats
)
from ai_analyzer import analyze_lecture, extract_full_text_from_pdf, fetch_image_for_keyword, clean_text, _detect_lecture_type
...
from payment_handler import (
    get_payment_keyboard, send_payment_required_message, handle_pay_stars,
    handle_pay_mastercard, handle_pay_crypto, handle_payment_sent, handle_pre_checkout,
...
def main_keyboard():
    return ReplyKeyboardMarkup([["📤 رفع محاضرة", "📊 رصيدي"], ["🔗 رابط الإحالة", "❓ مساعدة"]], resize_keyboard=True)
...
def _fmt_elapsed(sec):
    return f"{int(sec)} ثانية" if sec < 60 else f"{int(sec // 60)} دقيقة {int(sec % 60)} ثانية"

async def _safe_edit(msg, text, parse_mode="Markdown", reply_markup=None):
    try: await msg.edit_text(text, parse_mode=parse_mode, reply_markup=reply_markup)
    except: pass
...
async def ensure_user(update: Update):
    ...
    if not user:
        ...
        if ref_by and ref_by != tg.id: record_referral(ref_by, tg.id)
...
async def my_balance(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = await ensure_user(update)
    if not user: return
    await update.message.reply_text(f"💳 *رصيدك*\n\n🎬 المحاولات: *{user['attempts_left']}*\n📊 الفيديوهات: {user['total_videos']}", parse_mode="Markdown", reply_markup=get_payment_keyboard(user["user_id"]))
...
async def receive_content(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(uid) and await handle_admin_text_search(update, context): return
    ...
    if msg.text:
        ...
        if text == "📤 رفع محاضرة":
            ...
            return
        if text == "📊 رصيدي": await my_balance(update, context); return
        if text == "🔗 رابط الإحالة": await referral_cmd(update, context); return
        if text == "❓ مساعدة": await help_cmd(update, context); return

    if uid in _active_jobs:
        await msg.reply_text("⏳ جاري المعالجة..."); return

    lecture_text, filename = None, "lecture"
    if msg.document:
        fname = msg.document.file_name or ""
        ext = fname.lower().split(".")[-1] if "." in fname else ""
        if ext not in ("pdf", "txt"):
            await msg.reply_text("⚠️ PDF أو TXT فقط"); return
        wait = await msg.reply_text("📥 جاري قراءة الملف...")
        try:
            file = await msg.document.get_file()
            raw = await file.download_as_bytearray()
            if ext == "pdf": lecture_text = await extract_full_text_from_pdf(bytes(raw))
            else: lecture_text = raw.decode("utf-8", errors="ignore")
            filename = fname.replace(f".{ext}", "")
            await wait.delete()
        except Exception as e:
            await wait.edit_text(f"❌ خطأ: {e}"); return
    elif msg.text and len(msg.text.strip()) >= 200:
        lecture_text = msg.text.strip()
    else:
        await msg.reply_text("⚠️ أرسل PDF أو نص (200 حرف على الأقل)"); return

    lecture_text = clean_text(lecture_text)
    if not lecture_text or len(lecture_text) < 50:
        await msg.reply_text("❌ نص غير كاف"); return

    if user["attempts_left"] <= 0:
        await send_payment_required_message(update, context); return

    user_states[uid] = {"state": "awaiting_dialect", "text": lecture_text, "filename": filename}
    words = len(lecture_text.split())
    detected = _detect_lecture_type(lecture_text)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("admin_"): await handle_admin_callback(update, context); return
    await q.answer()
    if data == "pay_stars": await handle_pay_stars(update, context); return
    if data == "pay_mastercard": await handle_pay_mastercard(update, context); return
    if data == "pay_crypto": await handle_pay_crypto(update, context); return
    if data.startswith("sent_"): await handle_payment_sent(update, context); return
    if data == "cancel_job":
        ev = _cancel_flags.get(uid)
        if ev and not ev.is_set():
            ev.set()
    ...
    if data == "show_referral":
        ...
        bot = await context.bot.get_me()
        ref_link = f"https://t.me/{bot.username}?start=ref_{uid}"
        await q.message.reply_text(f"🔗 *رابطك*\n`{ref_link}`\n👥 {stats['total_referrals']} شخص", parse_mode="Markdown")
        return
    if data.startswith("dial_"):
        ...
        user = get_user(uid)
        if not user or user["attempts_left"] <= 0: await q.edit_message_text("❌ لا تملك محاولات."); return
        ...
        text, filename = state["text"], state.get("filename", "lecture")
        user_states.pop(uid, None)
...
async def _process_lecture(uid, text, filename, dialect, prog_msg, context):
    ...
    try:
        ...
        if not sections: raise RuntimeError("لم يتم استخراج أقسام")
        n_sec, lecture_type = len(sections), lecture_data.get("lecture_type", "other")
