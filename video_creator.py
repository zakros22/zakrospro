...
def _draw_text(d, x, y, text, font, color):
    text = _arabic(text)
    d.text((x+2, y+2), text, fill=(200,200,200), font=font)
...
def _welcome():
    ...
    x = (TARGET_W - w) // 2
    _draw_text(d, x, TARGET_H//2 - 40, WATERMARK, f, COLORS[0])
    f2 = _get_font(36)
    welcome = "أهلاً ومرحباً بكم"
    w2 = _text_width(_arabic(welcome), f2)
    x2 = (TARGET_W - w2) // 2
    _draw_text(d, x2, TARGET_H//2 + 30, welcome, f2, (44,62,80))
...
def _title(title):
    ...
    x = (TARGET_W - w) // 2
    _draw_text(d, x, TARGET_H//2 - 20, title, f, (44,62,80))
    img.save(p, "JPEG", quality=90)
    return p
...
def _sec_title(title, idx):
    ...
    x = (TARGET_W - w2) // 2
    _draw_text(d, x, cy + 50, title, f2, (44,62,80))
    img.save(p, "JPEG", quality=90)
    return p
...
def _content(img_bytes, keywords, sec_title, sec_idx, cur, total):
    ...
    hx = (TARGET_W - hw) // 2
    _draw_text(d, hx, 15, sec_title[:40], fh, (44,62,80))
...
def _summary(keywords):
    ...
    w = _text_width(_arabic(mt), f)
    x = (TARGET_W - w) // 2
    _draw_text(d, x, 35, mt, f, (44,62,80))
    y = 90
    f2 = _get_font(18)
    ...
    x3 = (TARGET_W - w3) // 2
    _draw_text(d, x3, TARGET_H - 60, th, f3, COLORS[0])
    img.save(p, "JPEG", quality=90)
    return p
...
def _ffmpeg_seg(img, dur, aud, start, out):
    ...
    if aud and os.path.exists(aud):
        cmd = ["ffmpeg", "-y", "-loop", "1", "-i", img, "-ss", f"{start:.3f}", "-t", dstr, "-i", aud,
               ...
               "-r", "15", "-c:a", "aac", "-b:a", "128k", "-ar", "44100", "-ac", "2", "-shortest", "-t", dstr, out]
    else:
