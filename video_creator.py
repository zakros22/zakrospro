...
def _create_content_slide(
    ...
    if image_bytes:
        try:
            ...
            draw.rectangle(
                [(20, img_area_top), (TARGET_W - 20, img_area_bottom)],
...
def _create_summary_slide(lecture_data: dict, sections: list, is_arabic: bool) -> str:
    ...
    for i, section in enumerate(sections[:4]):
        ...
        draw.rectangle([(x - 2, thumb_y - 2), (x + thumb_w + 2, thumb_y + thumb_h + 2)], fill=(40, 50, 70))
        draw.rectangle([(x, thumb_y), (x + thumb_w, thumb_y + thumb_h)], fill=(25, 35, 55), outline=ACCENT_COLORS[i % len(ACCENT_COLORS)], width=1)
        ...
        if img_bytes:
            try:
                ...
                pass
        draw.text((x + 5, thumb_y + thumb_h - 18), str(i + 1), fill=(255, 255, 255), font=_get_font(14, bold=True))
    ...
    key_points = lecture_data.get("key_points", [])[:4]
    if key_points:
